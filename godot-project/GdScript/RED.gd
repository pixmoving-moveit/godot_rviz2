...
extends TextureRect  # 继承自 TextureRect 类

var traffic_signal_report = TrafficLightReport  # 声明 TrafficLightReport 变量用于存储交通信号灯报告
var signal_states = []  # 存储交通信号灯状态的数组
var timer : Timer  # 定时器变量
...
func _process(delta: float) -> void:
	...
	if !traffic_signal_report.has_new():
		print("No new traffic light data.")  # 打印消息表示没有新的交通信号灯数据
		# self.visible = false
		return
...
func render_latest_signal_state():
	...
	if signal_states.size() > 0:
		...
		self.visible = (latest_state == 1)  # 如果最新状态为1，则设置节点可见，否则设置为不可见
	else:
		# 如果数组为空，设置节点不可见
		self.visible = false
